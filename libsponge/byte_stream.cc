...
size_t ByteStream::write(const string &data) {
    size_t canWriteLen = capacity - buffer.size();
    size_t realWriteLen = min(canWriteLen, data.length());
    for (size_t i = 0; i < realWriteLen; i++) {
        buffer.push_back(data[i]);
    }
    written_bytes += realWriteLen;
    return realWriteLen;
}

//! \param[in] len bytes will be copied from the output side of the buffer
string ByteStream::peek_output(const size_t len) const {
    size_t canPeekLen = min(len, buffer.size());
    string out = "";
    for (size_t i = 0; i < canPeekLen; i++) {
        out += buffer[i];
    }
    return out;
}
